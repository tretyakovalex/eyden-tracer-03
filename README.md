...
1. A new class ```CBoundingBox``` is now in the framwork which contains two ```Vec3f```’s for the ```min, max``` - fields of the
```CBoundingBox```.
...
3. The method ```virtual CBoundingBox CPrim::CalcBounds()``` has to be implemented in every class derived from ```CPrim```.
...
5. You will need a method to decide whether a primitive is contained in a given voxel or not. Therefore, a method ```bool CPrim::InVoxel(CBoundingBox& box)``` is added to the class ```CPrim```. Implement the method. For simplicity, just check the primitives bounding box for overlap with the box. Therefore, implement the method ```bool CBoundingBox::Overlaps(CBoundingBox&)``` in your box class. Keep in mind that floatingpoint numbers have limited accuracy!
6. Implement the method ```CScene::CalcBounds()```, which should calculate the bounding box of the scene.
